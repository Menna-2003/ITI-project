...
let containerYara = document.getElementsByClassName("container-yara");
...
darkMode.addEventListener("click", (e) => {
    ...
    for (let i = 0; i < containerYara.length; i++) {
        containerYara[i].style.color = threeSectionColor;
    }
...
LightMode.addEventListener("click", (e) => {
    ...
    window.localStorage.setItem("color", e.currentTarget.dataset.color);
    for (let i = 0; i < containerYara.length; i++) {
        containerYara[i].style.color = threeSectionLightColor;
    }
    body[0].style.backgroundColor = e.currentTarget.dataset.color;
